...
namespace Scheduler.Controllers
{
    public class ScheduleController : Controller
    {
        ...
        public static int reScheduleOrdertest(List<Order> orders)
        {
            int result = -1;
            ...
            for (int i = 0; i < orders.Count; i++)
            {
                ...
                for (int j = 0; j < MTData.Count; j++)
                {
                    ...
                    if (linePredictedSMTEnd < smtEndDate)
                    {
                        ...
                        schedule.latestStartDate = schedule.BEDate.AddSeconds(-lineSMTTime);
                    }
                    ...
                    int result2 = ScheduleProcessor.CreateSchedule(order.orderId, order.partId, fastestProcessingLine, schedule.backendId, schedule.earlistStartDate, schedule.earlistStartDate, schedule.plannedStartDate, schedule.latestStartDate, smtStartDate, smtEndDate);
                    if (result2 == 1)
                    {
                        //set order scheduled
                        OrdersController.setOrderscheduled(order.orderId);
        ...
        public static int scheduleOrdertest(Order order)
        {
